...
class BettingEnv(gym.Env):
    ...
    def _next_obs(self):
        ...
        print('Get next obs')
        # Get next game row
        obs = self.results.loc[self.game_number]
...
#         plt.clf()
    ...
    def step(self, action):
        ...
        # Done
        done = self.balance <= INITIAL_BALANCE / 2
    ...
    def reset(self):
        ...
        self.starting_point = np.random.randint(len(self.results) - len(self.results) * 0.1) # Start anywhere but in the end 10%
        self.timestep = 0
        self.games_won = 0
        self.game_bets = []
        self.game_number = self.starting_point + self.timestep
